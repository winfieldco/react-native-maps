...
const propTypes = {
   ...
   * used. Equivalent to icon in iOS, performant rendering of markers.
   *
   * @platform android
   */
  image: PropTypes.any,
  ...
  /**
   * Hide this marker when it goes off screen, this will help performance when there
   * are too many markers.
   */

  hidesOffScreen: PropTypes.bool,  
...
class MapMarker extends React.Component {
  ...
  updateOpacity(opacity) {
    this._runCommand('updateOpacity', [opacity]);
  }

  updateRotation(rotation) {
    this._runCommand('updateRotation', [rotation]);
  }
  ...
  render() {
    ...
    let iconSrc;
    if (this.props.icon) {
      iconSrc = resolveAssetSource(this.props.icon) || {};
      iconSrc = iconSrc.uri;
    }    
    ...
    return (
      <AIRMapMarker
        ...
        {...this.props}
        image={image}
        icon={iconSrc ? {src: iconSrc, size: this.props.iconSize} : undefined}
